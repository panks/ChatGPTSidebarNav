...
A lightweight Chrome extension that makes long ChatGPT chats easier to skim. It highlights every user question inside the conversation and adds a collapsible in-page sidebar with jump links so you can hop back to any question.

## Features

- Manual activation via the toolbar button keeps the site untouched until you need it.
- Adds a collapsible sidebar that lists every question detected in the current ChatGPT thread.
...
1. Navigate to `https://chatgpt.com` and open any conversation.
